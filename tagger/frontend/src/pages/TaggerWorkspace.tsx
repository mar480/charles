...
import ConceptOverridePanel from "@/components/tagger/ConceptOverridePanel";
...
const TaggerWorkspace = () => {
  ...
  const selectedField = useMemo(
    () => template?.fields.find((field) => field.fieldId === selectedFieldId),
  ...
  const handleBulkFieldInput = async (
    updates: Array<{ fieldId: string; rawValue: string; sourceType: string }>
  ) => {
    setProject((current) => {
      if (!current) {
        return current;
  ...
  const handlePreflight = async () => {
    ...
    try {
      ...
      setStatus(
        `Preflight completed with ${result.results.filter((entry) => entry.severity === "error").length} blocking issue(s).`
      );
    } catch (error) {
    ...
    <main className="tagger-page">
      <section className="tagger-hero">
        <div>
          ...
          <p className="tagger-muted">{[project.id, project.status, template.label].join(" · ")}</p>
        ...
        <div className="tagger-actions">
          <Link className="tagger-button secondary tagger-button-link" to={`/projects/${id}/setup`}>
            Back to setup
          </Link>
          <button type="button" className="tagger-button secondary" onClick={() => void handlePreflight()}>
            Run preflight
          </button>
          <button type="button" className="tagger-button primary" onClick={() => void handleGenerate()}>
            Generate iXBRL
          </button>
          <Link className="tagger-button secondary tagger-button-link" to={`/projects/${id}/validate`}>
            Validation review
          </Link>
        </div>
      </section>
      ...
      <section className="tagger-layout">
        <div className="tagger-main">
          <div className="tagger-card">
            ...
            <p className="tagger-muted">
              Paste a row or block from a spreadsheet into the selected template field to fill subsequent mapped fields
              in template order.
            </p>
          </div>
          ...
          <div className="tagger-card">
            ...
            <p className="tagger-muted">
              Preview before applying so the user can inspect matched fields, overwritten values, unmatched rows, and
              warnings.
            </p>
            <textarea value={csvContent} onChange={(event) => setCsvContent(event.target.value)} rows={8} />
            <div className="tagger-actions">
              <button type="button" className="tagger-button secondary" onClick={() => void handleImportPreview()}>
                Preview CSV import
              </button>
              <button type="button" className="tagger-button secondary" onClick={() => void handleImportApply()}>
                Apply CSV import
              </button>
            </div>
          </div>
          ...
          <div className="tagger-card">
            ...
            <p className="tagger-muted">
              The first worksheet is read into the template mapping using `field`/`value`-style headers.
            </p>
            <input
              type="file"
              accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
              onChange={(event) => void handleXlsxFile(event.target.files?.[0] || null)}
            />
            <div className="tagger-actions">
              <button
                type="button"
                className="tagger-button secondary"
                onClick={() => void handleXlsxPreview()}
                disabled={!xlsxBase64}
              >
                Preview XLSX import
              </button>
        ...
        <aside className="tagger-sidebar">
          ...
          />
          <ConceptOverridePanel
            field={selectedField}
            currentOverrideQname={
              selectedFieldId ? String(project.fields[selectedFieldId]?.conceptOverrideQname || "") : ""
            }
